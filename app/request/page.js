...
import { ShieldCheck, Clock, Sparkles, CheckCircle2, CalendarDays } from 'lucide-react';
import { useEffect, useState } from 'react';

export default function Request() {
  const [calendlyLoaded, setCalendlyLoaded] = useState(false);

  // AOS nur clientseitig
  useEffect(() => {
    let cleanup = () => {};
    (async () => {
  ...
  return (
    <div className="font-proxima bg-[#0C0D12] text-white">
      ...
      >
        ...
        <div className="relative px-5 md:px-20 py-16 md:py-24">
          <div className="max-w-4xl">
            ...
            <p
              className="mt-4 max-w-2xl text-[#C9CEE0]"
              data-aos="fade-up"
              data-aos-delay="80"
            >
              30 Minuten Fokus: Ziele verstehen, Ansatz skizzieren, Timing & Pakete
              grob einordnen. Unverbindlich, effizient, planbar.
            </p>
      ...
      <section className="px-5 md:px-20 py-10 md:py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <StepCard
            ...
            text="Wir ordnen Module (Branding, Web, Social, Systeme) sinnvoll & schlank."
            delay={60}
          />
          <StepCard
            index="03"
      ...
      <section className="px-5 md:px-20 pb-16 md:pb-24">
        <div
          className="rounded-3xl border border-white/10 bg-white/[0.04] backdrop-blur p-3 md:p-6"
          data-aos="fade-up"
        >
          {/* Fallback-Skelett beim Laden */}
          {!calendlyLoaded && (
        ...
        <div className="mt-6 flex flex-col md:flex-row items-start md:items-center gap-3 text-sm text-[#AEB5C8]">
          ...
          <p>
            Deine Daten werden nur zur Terminabstimmung genutzt. Keine Newsletter,
            kein Spam. Alternativ:{" "}
