...
function Badge({ status, lang }) {
  ...
  return <span className={cx("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border", m.bg, m.color)}><span className={cx("w-1.5 h-1.5 rounded-full shrink-0", m.dot)} />{label}</span>;
}
    ...
    <div className="overflow-x-auto">
      <div className="min-w-[460px]">
        ...
        <div className="relative space-y-2">
          <div className="absolute top-0 bottom-0 w-px bg-violet-400/60 z-10" style={{ left: `calc(7rem + ${tPct}% * (100% - 7rem) / 100)` }}>
            <div className="absolute -top-1 left-1/2 -translate-x-1/2 text-[9px] text-violet-300 whitespace-nowrap">{T[lang].today}</div>
          </div>
          {sprints.map(s => {
              ...
              <div key={s.id} className="flex items-center gap-3">
                ...
                <div className="flex-1 relative h-8">
                  ...
                  <div className={cx("absolute top-1 bottom-1 rounded-full border", bc[s.status]??bc.pending)} style={{ left:`${left}%`, width:`${width}%`, minWidth:"2rem" }}>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center"><span className="text-[10px] font-semibold text-white/80 truncate px-2">{t(s.title, lang)}</span></div>
                  </div>
                </div>
              </div>
            );
      ...
      <Card glow={sprint.status==="active"} className="overflow-hidden">
        <div className="p-5 md:p-6 cursor-pointer select-none" onClick={()=>setCollapsed(c=>!c)}>
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-3 flex-wrap"><Badge status={sprint.status} lang={lang} /><span className="text-xs text-white/40">{sprint.label}</span></div>
            <div className="flex items-center gap-2">
              {sprint.payment&&<span className="text-xs text-white/45 bg-white/6 border border-white/10 rounded-full px-2.5 py-1">{t(sprint.payment, lang)} · {sprint.payment.amount} €</span>}
          ...
          <div className="mt-3">
            ...
            <div className="mt-1 text-xs text-white/35">{formatDate(sprint.startDate)} – {formatDate(sprint.endDate)} · {doneCount}/{sprint.tasks.length} {tr.tasks}</div>
          </div>
        </div>
        {!collapsed&&(
          <div className="border-t border-white/8 px-5 md:px-6 pb-5 pt-4">
            {editMode&&(
              <div className="mb-4 flex flex-wrap gap-2">
                {["done","active","pending"].map((s,i)=><button key={s} onClick={()=>onStatusChange(sprint.id,s)} className={cx("text-xs px-3 py-1 rounded-full border transition-colors",sprint.status===s?"border-white/30 bg-white/15 text-white":"border-white/10 bg-white/4 text-white/50 hover:bg-white/10")}>{tr.sprintStatuses[i]}</button>)}
              </div>
            )}
            <div className="space-y-2">
              {sprint.tasks.map(task=>(
                <div key={task.id} className={cx("flex items-center gap-3 rounded-xl border px-3 py-2.5 transition-all",task.status==="done"?"border-emerald-300/15 bg-emerald-500/6":"border-white/9 bg-white/4")}>
                  ...
                  <span className={cx("flex-1 text-sm",task.status==="done"?"text-white/40 line-through":"text-white/80")}>{t(task.title, lang)}</span>
...
export default function OksanaHettingerPage() {
  const [project,   setProject]   = useState(() => { try { const s = localStorage.getItem(STORAGE_KEY); return s ? JSON.parse(s) : INITIAL_PROJECT; } catch { return INITIAL_PROJECT; } });
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-black/25 border-b border-white/8">
        <div className="max-w-6xl mx-auto px-5 md:px-10 h-14 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div><span className="text-sm font-bold text-white/90">{t(project.client, lang)}</span><span className="mx-2 text-white/20 text-sm">/</span><span className="text-sm text-white/55">{tr.headerSub}</span></div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setEditMode(e => !e)} className={cx("inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-semibold border transition-all", editMode ? "border-violet-300/40 bg-violet-500/20 text-violet-200" : "border-white/12 bg-white/6 text-white/60 hover:bg-white/10")}>
              <span className={cx("w-1.5 h-1.5 rounded-full", editMode ? "bg-violet-400 animate-pulse" : "bg-white/30")} />{editMode ? tr.editOn : tr.editOff}
            </button>
            {editMode && <button onClick={() => { setProject(INITIAL_PROJECT); try { localStorage.removeItem(STORAGE_KEY); } catch {} }} className="px-3 py-1.5 rounded-full text-xs border border-white/10 bg-white/4 text-white/35 hover:bg-white/8 transition-colors">{tr.reset}</button>}
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-5 md:px-10 py-8 md:py-12 space-y-8">
        ...
        <Reveal>
          <div className="rounded-2xl border border-amber-300/20 bg-amber-500/8 px-5 py-4 flex items-center gap-3">
            ...
            <p className="text-sm text-white/75"><span className="font-semibold text-white/90">{tr.banner.bold}</span>{tr.banner.rest}</p>
          </div>
        </Reveal>

        <Reveal>
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm"><span className="text-white/55">{tr.overall}</span><span className="font-bold text-white/90">{project.overallProgress}%</span></div>
        ...
        <Reveal delay={100}>
          <div className="flex gap-1 bg-white/5 border border-white/10 rounded-2xl p-1 w-fit flex-wrap">
            {tr.tabs.map((label, i) => {
              ...
              return <button key={ids[i]} onClick={() => setActiveTab(ids[i])} className={cx("px-4 py-2 rounded-xl text-sm font-semibold transition-all", activeTab === ids[i] ? "bg-white/15 text-white shadow-sm" : "text-white/45 hover:text-white/70")}>{label}</button>;
            })}
          </div>
        </Reveal>
        ...
        {activeTab === "sprints" && (
          <div className="space-y-4">
            {project.sprints.map(s => <SprintCard key={s.id} sprint={s} editMode={editMode} onTaskToggle={handleTaskToggle} onStatusChange={handleSprintStatus} lang={lang} />)}
          </div>
        )}

        {activeTab === "gantt" && (
          <Reveal><Card className="p-5 md:p-7"><SLabel>{tr.timeline}</SLabel><div className="mt-1 text-lg font-bold text-white/88 mb-6">{formatDate(project.startDate)} – {formatDate(project.deadline)}</div><GanttChart sprints={project.sprints} start={project.startDate} end={project.deadline} lang={lang} /></Card></Reveal>
        )}

        {activeTab === "milestones" && (
          <Reveal><Card className="p-5 md:p-7"><SLabel>{tr.milestones}</SLabel><div className="mt-1 text-lg font-bold text-white/88 mb-6">{editMode && <span className="text-xs text-violet-300 font-normal ml-2">{tr.editHint}</span>}</div><Milestones milestones={project.milestones} editMode={editMode} onToggle={handleMilestone} lang={lang} /></Card></Reveal>
        )}
              ...
              <div className="space-y-3">
                {project.sprints.filter(s => s.payment).map(s => (
                  <div key={s.id} className={cx("rounded-2xl border px-4 py-4 flex items-center justify-between gap-3", s.status==="done"?"border-emerald-300/20 bg-emerald-500/8":s.id==="s0"?"border-amber-300/20 bg-amber-500/8":"border-white/10 bg-white/4")}>
                    <div><div className="text-xs text-white/40">{s.label}</div><div className="text-sm text-white/80 font-semibold mt-0.5">{t(s.payment, lang)}</div></div>
                    <div className="flex items-center gap-3"><span className="text-xl font-extrabold text-white/90">{s.payment.amount} €</span><Badge status={s.status} lang={lang} /></div>
                  </div>
                ))}
        ...
        <Reveal delay={200}>
          <div className="text-xs text-white/25 text-center pb-8">Leon Seitz · {tr.footer} · <a href="https://leonseitz.com" className="underline underline-offset-2 hover:text-white/45 transition-colors">leonseitz.com</a></div>
        </Reveal>
      </main>
    </div>
  );
