...
import { Palette, MessageSquare, Sparkles, ArrowRight, CheckCircle2 } from 'lucide-react';
...
export default function Content() {
  ...
  const modules = [
    'Content‑Plan (Formate, Hooks, Frequenzen)',
    'Copywriting & Visual Guidelines',
    'Assets: Posts, Reels, Ads, Landing‑Visuals',
    'Short‑Form Scripts & Hook‑Library',
    'Distribution & Iteration nach Daten',
  ];
  ...
  return (
    <div className="font-proxima bg-[#0B0B0F] text-white">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative px-5 md:px-16 pt-20 md:pt-28 pb-16 text-center max-w-4xl mx-auto">
          <span className="inline-flex items-center gap-2 text-sm text-indigo-300/80 bg-white/5 ring-1 ring-white/10 px-3 py-1 rounded-full">
            <MessageSquare size={16} /> Content Creation
          </span>
          <h1 className="mt-6 text-4xl md:text-6xl font-extrabold tracking-tight">
            Content, der <span className="text-indigo-300">performt</span>.
          </h1>
          <p className="mt-6 text-lg md:text-xl text-neutral-300">
            Klare Ideen, klare Hooks, klare Wirkung – zugeschnitten auf Ziele & Funnel.
          </p>
        </div>
      </section>
      ...
      <section className="px-5 md:px-16 py-12">
        <h2 className="text-2xl md:text-3xl font-semibold mb-6">Bausteine</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          {modules.map((m, i) => (
            <div key={i} className="rounded-xl border border-white/10 bg-white/[0.04] p-4" data-aos="fade-up" data-aos-delay={i * 80}>
              ...
              <div className="text-neutral-200">{m}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-5 md:px-16 py-10 md:py-16 text-center">
        <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-white/[0.04] to-white/[0.02] p-10">
          <h3 className="text-3xl md:text-4xl font-bold">Mehr Reichweite ist gut. Mehr Anfragen ist besser.</h3>
          <p className="mt-3 text-neutral-300">Wir binden Content direkt in Funnel & CRM ein.</p>
